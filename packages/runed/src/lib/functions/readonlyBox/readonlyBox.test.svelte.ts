...
import { type ReadonlyBox, type ValueOrReadonlyBox, readonlyBox } from "./readonlyBox.svelte.js";
...
describe("readonlyBox types", () => {
	...
	test("readonlyBox of ValueOrReadonlyBox", () => {
		const count = undefined as unknown as ValueOrReadonlyBox<number>; // typing this as number casts it
		const readonlyCount = readonlyBox(count);
		expectTypeOf(readonlyCount).toMatchTypeOf<ReadonlyBox<number>>;
	});
});
